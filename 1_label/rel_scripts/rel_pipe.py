...
def span_tuple_annot(e1, e2):
    span_pair = (e1.start, e1.end, e1.label_, e2.start, e2.end, e2.label_)
    return span_pair
    ...
    def set_annotations(self, docs: Iterable[Doc], scores: Floats2d) -> None:
        ...
        for doc in docs:
            for (e1, e2) in get_instances(doc):
                span_pair = span_tuple_annot(e1, e2) # TODO: check this
                if span_pair not in doc._.rel:
                    doc._.rel[span_pair] = {}
                for j, rel_label in enumerate(self.labels):
                    doc._.rel[span_pair][rel_label] = scores[c, j]
                c += 1
    ...
    def _examples_to_truth(self, examples: List[Example]) -> Optional[numpy.ndarray]:
        ...
        for i, eg in enumerate(examples):
            for (e1, e2) in self.model.attrs["get_instances"](eg.reference):
                gold_label_dict = eg.reference._.rel.get(span_tuple_annot(e1, e2), {})
                for j, rel_label in enumerate(self.labels):
                    truths[c, j] = gold_label_dict.get(rel_label, 0)
                c += 1
